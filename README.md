...
+ **Coin Type:** This field indicates which cryptocurrency is being used in a multi-currency wallet. All coins, including testnet bitcoin, are assigned [a constant number](https://github.com/satoshilabs/slips/blob/master/slip-0044.md). For example, a derivation path for a Monero (XMR) account would be `m/44'/128'`. *Note that if finalized, [BIP45](https://github.com/bitcoin/bips/blob/master/bip-0045.mediawiki) would designate this level as the 'Cosigner Index' instead.*
+ **Account:** This field, in a multi-account wallet, indicates the identity or collection of addresses, which allows users to segregate funds for different things (ex. savings, donations). *Note that if finalized, BIP45 would not include this field. If [BIP47](https://github.com/bitcoin/bips/blob/master/bip-0047.mediawiki) is finalized, this level would be designated as 'Identity', though it is equivalent to 'Account.'*
+ **Change:** This field, if the constant `0` is present, indicates "external chain" (regular) addresses; if the constant `1`, indicates "internal chain" or change addresses. *Note that if finalized, [BIP47](https://github.com/bitcoin/bips/blob/master/bip-0047.mediawiki) would designate this level as space for the notification keys and ephemeral payment codes.*
