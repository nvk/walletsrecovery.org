...
// Dynamic Featured Wallets Functions
function calculateWalletScore(wallet) {
  let score = 0;
  
  // Documentation score (highest weight)
  if (wallet.status.includes('✅')) score += 50;
  if (wallet.status.includes('⚠️')) score += 20;
  if (wallet.status.includes('☠️')) score -= 30;
  if (wallet.status.includes('😵')) score -= 50;
  
  // Security concerns (negative scoring)
  if (wallet.status.includes('🚸')) score -= 40;
  if (wallet.status.includes('👁')) score -= 10;
  if (wallet.status.includes('⑂')) score -= 10;
  
  // New team penalty
  if (wallet.status.includes('🧐')) score -= 5;
  
  // Derivation path support
  if (wallet.derivation.bip44) score += 5;
  if (wallet.derivation.bip49) score += 5;
  if (wallet.derivation.bip84) score += 10;
  if (wallet.derivation.bip86) score += 10;
  if (wallet.derivation.bip48) score += 15;
  
  // Feature support
  if (wallet.bip174_psbt === 'Yes') score += 15;
  if (wallet.output_descriptor === 'Yes') score += 10;
  if (wallet.bip39_pass === 'Yes' || wallet.bip39_pass === 'Optional') score += 5;
  if (wallet.wif_support === 'Yes') score += 5;
  
  // Model preference bonus (newer models get slight preference when features are identical)
  if (wallet.name.includes('ColdCard')) {
    if (wallet.name.includes('Q')) score += 3;        // ColdCard Q (newest)
    else if (wallet.name.includes('MK4')) score += 2; // ColdCard MK4
    else if (wallet.name.includes('MK3')) score += 1; // ColdCard MK3 (oldest)
  }
  
  // Trezor model preference
  if (wallet.name.includes('Trezor')) {
    if (wallet.name.includes('Model T')) score += 2;  // Trezor Model T
    else if (wallet.name.includes('Model One')) score += 1; // Trezor Model One
  }
  
  // Ledger model preference
  if (wallet.name.includes('Ledger')) {
    if (wallet.name.includes('Nano X')) score += 2;   // Ledger Nano X
    else if (wallet.name.includes('Nano S')) score += 1; // Ledger Nano S
  }
  
  return score;
}

function getBestOverallWallets(walletsData, limit = 3) {
  const allWallets = [];
    ...
    if (category.name !== 'Lightning Wallets') {
      category.wallets.forEach(wallet => {
        allWallets.push({
          ...
          score: calculateWalletScore(wallet)
        });
      });
    }
  });
...
function getPSBTWallets(walletsData, limit = 3) {
  const psbtWallets = [];
...
function getLightningWallets(walletsData, limit = 3) {
  const lightningCategory = walletsData.categories.find(cat => cat.name === 'Lightning Wallets');
  if (!lightningCategory) return [];
  
  return lightningCategory.wallets
    .filter(wallet => !wallet.status.includes('😵')) // Exclude discontinued
    .slice(0, limit);
}

function getMobileWallets(walletsData, limit = 3) {
  const mobileWallets = [];
  
  walletsData.categories.forEach(category => {
    if (category.name === 'Software Wallets') {
      category.wallets.forEach(wallet => {
        // Look for mobile indicators in name or notes
        const notesText = wallet.notes ? (typeof wallet.notes === 'string' ? wallet.notes : JSON.stringify(wallet.notes)) : '';
        const isMobile = wallet.name.toLowerCase().includes('mobile') ||
                        wallet.name.toLowerCase().includes('app') ||
                        notesText.toLowerCase().includes('mobile') ||
                        notesText.toLowerCase().includes('ios') ||
                        notesText.toLowerCase().includes('android');
        
        if (isMobile && !wallet.status.includes('😵')) {
          mobileWallets.push({
            ...wallet,
            category: category.name,
            score: calculateWalletScore(wallet)
...
function getAvoidWallets(walletsData, limit = 3) {
  const avoidWallets = [];
  
  walletsData.categories.forEach(category => {
    if (category.name !== 'Lightning Wallets') {
      category.wallets.forEach(wallet => {
        // Wallets to avoid based on status
        const shouldAvoid = wallet.status.includes('☠️') || // Not publicly available
                           wallet.status.includes('😵') || // Discontinued
                           wallet.status.includes('🚸');   // Physically unsafe
...
function renderDynamicFeaturedWallets(walletsData) {
  const featuredGrid = document.querySelector('.featured-grid');
  if (!featuredGrid) return;
  
  try {
    const avoidWallets = getAvoidWallets(walletsData);
    const bestOverall = getBestOverallWallets(walletsData);
    const psbtWallets = getPSBTWallets(walletsData);
    const lightningWallets = getLightningWallets(walletsData);
    const mobileWallets = getMobileWallets(walletsData);
  
  const categories = [
    {
      title: '⚠️ Avoid for Recovery',
      wallets: avoidWallets,
      isWarning: true
    },
    {
      title: '🏆 Best Overall',
  ...
  featuredGrid.innerHTML = categories.map(category => {
    const walletLinks = category.wallets.map(wallet => 
      `<a href="#wallet-tables" class="wallet-link" data-wallet="${wallet.name}" onclick="scrollToWallet('${wallet.name}')">${wallet.name}</a>`
    ).join('');
    
    const warningClass = category.isWarning ? ' warning-card' : '';
    
    return `
      <div class="featured-card${warningClass}">
    ...
    console.error('Error rendering featured wallets:', error);
    // Fallback to static content if dynamic loading fails
    featuredGrid.innerHTML = `
      <div class="featured-card warning-card">
        <h3>⚠️ Avoid for Recovery</h3>
        <div class="wallet-list">
          <a href="#wallet-tables" class="wallet-link">Check wallet tables for discontinued wallets</a>
        </div>
      </div>
      <div class="featured-card">
        <h3>🏆 Best Overall</h3>
        <div class="wallet-list">
          <a href="#wallet-tables" class="wallet-link">ColdCard Series</a>
          <a href="#wallet-tables" class="wallet-link">Trezor Model T</a>
          <a href="#wallet-tables" class="wallet-link">Ledger Nano X</a>
        </div>
      </div>
      <div class="featured-card">
        <h3>🔧 PSBT Support</h3>
        <div class="wallet-list">
          <a href="#wallet-tables" class="wallet-link">ColdCard Series</a>
          <a href="#wallet-tables" class="wallet-link">Jade</a>
          <a href="#wallet-tables" class="wallet-link">SeedSigner</a>
        </div>
      </div>
      <div class="featured-card">
        <h3>⚡ Lightning Ready</h3>
        <div class="wallet-list">
          <a href="#wallet-tables" class="wallet-link">Mutiny Wallet</a>
          <a href="#wallet-tables" class="wallet-link">Zeus LN</a>
          <a href="#wallet-tables" class="wallet-link">Blixt</a>
        </div>
      </div>
      <div class="featured-card">
        <h3>📱 Mobile Apps</h3>
        <div class="wallet-list">
          <a href="#wallet-tables" class="wallet-link">BlueWallet</a>
          <a href="#wallet-tables" class="wallet-link">Blockstream Green</a>
          <a href="#wallet-tables" class="wallet-link">Cove Wallet</a>
        </div>
      </div>
    `;
  }
...
// Function to scroll to specific wallet in the table
function scrollToWallet(walletName) {
  // Find the wallet row in the table
  const walletRows = document.querySelectorAll('.wallet-table tbody tr');
  let targetRow = null;
  
  walletRows.forEach(row => {
    const nameCell = row.querySelector('td:nth-child(10)'); // Assuming name is in 10th column
    if (nameCell && nameCell.textContent.trim().includes(walletName)) {
      targetRow = row;
    }
  });
  ...
  if (targetRow) {
    ...
    targetRow.style.backgroundColor = 'rgba(22, 82, 240, 0.1)';
    setTimeout(() => {
  ...
  fetch('walletsrecovery.json?v=202501122200', {
    ...
    .then(walletsData => {
      ...
      requestAnimationFrame(() => {
        ...
        // Render featured wallets after a short delay to ensure DOM is ready
        setTimeout(() => {
          console.log('About to render featured wallets...');
          try {
            renderDynamicFeaturedWallets(walletsData);
          } catch (error) {
            console.error('Error in renderDynamicFeaturedWallets:', error);
            ...
            if (featuredGrid) {
              featuredGrid.innerHTML = `
                ...
                <div class="featured-card">
                  <h3>🏆 Best Overall</h3>
                  <div class="wallet-list">
                    <a href="#wallet-tables" class="wallet-link">ColdCard Q</a>
                    <a href="#wallet-tables" class="wallet-link">ColdCard MK4</a>
                    <a href="#wallet-tables" class="wallet-link">Trezor Model T</a>
                  </div>
                </div>
                <div class="featured-card">
